...
const SkillAnalysisModule = {
  ...
  readinessCache: null,
  industrySkillsCache: null,

  /**
   * Asynchronously loads career_readiness.json
   */
  async loadReadinessFramework(url = 'career_readiness.json') {
    if (this.readinessCache) return this.readinessCache;
    try {
      const res = await fetch(url);
      if (res.ok) {
        this.readinessCache = await res.json();
        return this.readinessCache;
      }
    } catch (e) {
      console.warn("SkillAnalysisModule: Could not load career_readiness.json", e);
    }
    return null;
  },

  /**
   * Asynchronously loads industry_skills.json
   */
  async loadIndustrySkills(url = 'industry_skills.json') {
    if (this.industrySkillsCache) return this.industrySkillsCache;
    try {
      const res = await fetch(url);
      if (res.ok) {
        this.industrySkillsCache = await res.json();
        return this.industrySkillsCache;
      }
    } catch (e) {
      console.warn("SkillAnalysisModule: Could not load industry_skills.json", e);
    }
    return null;
  },
